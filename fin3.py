    ...
    for i in range(1,nx-1):
        for j in range(ny):
            if j==0:
                ...
                lap=(T[i-1][j][2]+T[i+1][j][2]-2*T[i][j][2])/(ds**2) + (1/(j*ds))*(T[i][j][2]-T[i][j-1][2])/(2*ds) + (T[i][j-1][2]-2*T[i][j][2]+T[i][j][2])/(ds**2)
                T[i][ny-1][2]+=(alp*lap+h * (Tinf - T[i][0][2]))*dt
                
            else:
                lap=(T[i-1][j][2]+T[i+1][j][2]-2*T[i][j][2])/(ds**2) + (1/(j*ds))*(T[i][j][2]-T[i][j-1][2])/(2*ds) + (T[i][j-1][2]-2*T[i][j][2]+T[i][j+1][2])/(ds**2)
                T[i][j][2]+=alp*lap*dt
